...
конфіг з токеном, ролі, незавершені перевірки, журнал завершених.
...
def конфіг():
    ...
    try:
        ...
        pass
    з_файлу.update({k: v for k, v in os.environ.items() if k in з_файлу or k.startswith(("TELEGRAM_", "SHOPPER_", "REPORT_"))})
    return з_файлу
...
def json_писати(імя, дані):
    ...
    # запису, на диску залишиться стара цілa версія, а не половина нової.
    тимч = шлях(імя) + ".tmp"
